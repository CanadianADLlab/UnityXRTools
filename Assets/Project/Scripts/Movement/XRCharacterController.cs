...
public class XRCharacterController : MonoBehaviour
{
    ...
    {
        if(XRPositionManager.Instance.LeftHand.GetComponent<XRController>().IsGrabbing && !ignoringLeftHand)
        {
            print("Ignoring Colliders Left");
            IgnoreChildrenColliders(XRPositionManager.Instance.LeftHand.transform);
            ignoringLeftHand = true;
        }
        else if(ignoringLeftHand && !XRPositionManager.Instance.LeftHand.GetComponent<XRController>().IsGrabbing)
        {
            print("Enable Colliders Left");
            ignoringLeftHand = false;
            IgnoreChildrenColliders(XRPositionManager.Instance.LeftHand.transform, ignoringLeftHand); // Passing false re enables zee collider
        }
        print("Right is grabbing " + XRPositionManager.Instance.RightHand.GetComponent<XRController>().IsGrabbing);
        print("Ignoring hand " + ignoringRightHand);
        if (XRPositionManager.Instance.RightHand.GetComponent<XRController>().IsGrabbing && !ignoringRightHand)
        {
            print("Ignoring Colliders Right");
            IgnoreChildrenColliders(XRPositionManager.Instance.RightHand.transform);
            ignoringRightHand = true;
        }
        else if(ignoringRightHand && !XRPositionManager.Instance.RightHand.GetComponent<XRController>().IsGrabbing)
        {
            print("Enable Colliders Right");
            ignoringRightHand = false;
            IgnoreChildrenColliders(XRPositionManager.Instance.RightHand.transform, ignoringRightHand);
        }
    }
